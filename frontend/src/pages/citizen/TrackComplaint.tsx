...
function ComplaintCard({ c: initialC }: { c: Complaint }) {
  ...
  // Show re-verification banner when resolved and citizen hasn't responded yet
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
      ...
      {images.length > 0 && (
        <div className="flex gap-2 mt-2 mb-3 overflow-x-auto pb-1">
          {images.map((m, i) => (
            <a key={i} href={`http://localhost:8000/media/${m.file_path.split(/[\\/]/).pop()}`} target="_blank" rel="noreferrer">
              <img
                src={`http://localhost:8000/media/${m.file_path.split(/[\\/]/).pop()}`}
                alt={m.original_filename || 'complaint image'}
                className="h-28 w-40 object-cover rounded-lg border border-gray-200 flex-shrink-0 hover:opacity-90 transition"
              />
            </a>
          ))}
        </div>
