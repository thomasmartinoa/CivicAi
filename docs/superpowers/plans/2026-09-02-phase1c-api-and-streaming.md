...
from app.services.media import (
    ALLOWED_MEDIA, MAX_UPLOAD_BYTES, MediaTooLarge, MediaTypeNotAllowed,
    UPLOAD_ROOT, store_upload,
)


def test_a_stored_file_lands_under_the_upload_root():
    stored = store_upload(b"fake-jpeg-bytes", "photo.jpg")
    assert (UPLOAD_ROOT / Path(stored.file_path).name).exists()
    (UPLOAD_ROOT / Path(stored.file_path).name).unlink()


def test_the_stored_name_is_generated_not_client_supplied():
...
def test_the_stored_path_is_relative_and_prefixed():
...
def test_extension_determines_media_type():
    for ext, expected in (("jpg", "image"), ("png", "image"), ("mp3", "voice")):
        stored = store_upload(b"x", f"f.{ext}")
        assert stored.media_type == expected
        (UPLOAD_ROOT / Path(stored.file_path).name).unlink()


def test_a_disallowed_type_is_rejected_before_anything_is_written():
...
def test_two_uploads_of_the_same_name_do_not_collide():
    ...
    for s in (a, b):
        (UPLOAD_ROOT / Path(s.file_path).name).unlink()
```
...
Expected: PASS — the 5 pre-existing seed tests plus 12 new ones.
